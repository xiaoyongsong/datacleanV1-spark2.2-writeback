...
object MFD_2TO1V1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("MultiFDRepair").set("spark.network.timeout","36000").
      set("spark.sql.broadcastTimeout","36000").set("spark.executor.heartbeatInterval","200")
    ...
    //val readSql = "select * from "+tableName
   ...
   println("读到的HDFS数据：")
    ...
    val tableName2 = "dataquality_"+tableName + "_" + ruleType + "_" + System.currentTimeMillis
    println("start writing back result at " + Calendar.getInstance().getTime)

    println("TableName: "+tableName1+"\n修复的写回tableName:"+tableName2)

    //换写回的数据库
    //sqlContext.sql("CREATE TABLE IF NOT EXISTS sparkrepair." + tableName1 + " as select * from repairResult")
    sqlContext.sql("CREATE TABLE IF NOT EXISTS default." + tableName2 + " as select * from repairResult")   //修复的结果
    sqlContext.sql("CREATE TABLE IF NOT EXISTS sparkdb." + tableName1 + " as select * from temp")    //检测的结果

...
object DCDetect {
  ...
  def main(args: Array[String]): Unit = {
   ...
   // val args =Array("wonders_dataqualitytest","dc","A.city=B.city@A.zip!=B.zip@A.salary>15000")
    ...
    println("tableName: "+tableName+" ruleType: "+ruleType+" ruleContent:"+ruleContent)
    ...
    val originalSql = "select "+originalSelect+" from default."+tableName
    ...
    var executeSql=""
     if(sqlStr.length>0){            //说明有涉及两元组的过滤条件
      // var newDF1 = sqlContext.sql("select "+selectStr+" from A,B where "+sqlStr)
       ...
       executeSql = "select count(*) from A,B where "+sqlStr
       ...
       executeSql = "select count(*) from A "
